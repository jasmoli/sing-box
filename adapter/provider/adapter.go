...
func (a *Adapter) UpdateOutbounds(oldOpts []option.Outbound, newOpts []option.Outbound) {
	...
	var (
		...
		tagCount       = make(map[string]uint16)
	)
	...
	for i, opt := range newOpts {
		...
		tagCount[tag]++
		if tagCount[tag] > 1 {
			tag = F.ToString(tag, "(", tagCount[tag], ")")
		}
	...
	if len(outbounds) == 0 {
		err := a.outbound.Create(
			...
			a.router,
			a.logFactory.NewLogger(F.ToString("outbound/", C.TypeDirect, "[", "OUTBOUNDLESS", "]")),
			"OUTBOUNDLESS",
			C.TypeDirect,
			&option.DirectOutboundOptions{},
		)
		if err != nil {
			a.logger.Error(err, " in ", "OUTBOUNDLESS", ", skip create this outbound")
			return
		}
		outbound, _ := a.outbound.Outbound("OUTBOUNDLESS")
		outbounds = append(outbounds, outbound)
		outboundsByTag["OUTBOUNDLESS"] = outbound
	}
