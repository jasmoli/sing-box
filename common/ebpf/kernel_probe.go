...
// KernelProbeDataPlane selects the implementation probed by ProbeKernel. The TC
// data plane is preferred; the cgroup data plane is the legacy fallback used on
// kernels without the TC helpers (notably Linux below 5.10).
...
type KernelProbeOptions struct {
	...
	DataPlane           KernelProbeDataPlane
	Network             []string
...
func ProbeKernel(options KernelProbeOptions) (*KernelProbeReport, error) {
	...
	needShared := options.Mode == KernelProbeModeAll || options.Mode == KernelProbeModeShared
	if options.DataPlane == KernelProbeDataPlaneCgroup {
		probeCgroupCommonCapabilities(report, memlockErr)
		if needLocal {
			probeCgroupLocalCapabilities(report, options.CgroupPath, enableTCP, enableUDP)
		}
		if needShared {
...
// --- legacy cgroup data plane probes (Linux < 5.10 fallback) ---

func probeCgroupCommonCapabilities(report *KernelProbeReport, memlockErr error) {
	if os.Geteuid() == 0 {
	...
	probeMapType(report, "common", KernelProbeRequired, CiliumEBPF.LRUHash,
		"Stores bounded socket, UDP, and bypass caches.")
	probeMapType(report, "common", KernelProbeRequired, CiliumEBPF.LPMTrie,
...
func probeCgroupLocalCapabilities(report *KernelProbeReport, configuredPath string, enableTCP bool, enableUDP bool) {
	const scope = "local"
	...
	for _, helper := range []struct {
		...
		{asm.FnGetSocketCookie, "bpf_get_socket_cookie", "Identifies sockets and enforces sing-box self-protection."},
...
func probeSharedNetworkCapabilities(report *KernelProbeReport, interfaceName string) {
	...
	for _, helper := range []struct {
		...
		{asm.FnMapUpdateElem, "bpf_map_update_elem", "Creates proxy, reply, and bypass state."},
		{asm.FnMapDeleteElem, "bpf_map_delete_elem", "Removes expired or conflicting flow state."},
		{asm.FnKtimeGetNs, "bpf_ktime_get_ns", "Applies UDP, TCP, and bypass cache lifetimes."},
	...
	probeSharedNetworkInterface(report, interfaceName)
}

func probeSharedNetworkInterface(report *KernelProbeReport, interfaceName string) {
	const scope = "shared-network"
	if interfaceName == "" {
