...
// Connection handling for the legacy cgroup data plane.
...
func (i *Inbound) closeCgroupListeners() error {
	return i.listeners.close()
}

func (i *Inbound) newCgroupConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
...
func (i *Inbound) newCgroupPacket(buffer *buf.Buffer, oob []byte, source M.Socksaddr) {
	...
	redirectAddress, err := cgroupRedirectAddressFromOOB(oob)
	if err != nil {
	...
	cached, bindingReady, loaded := i.cgroupUDPClientTable.cachedPacketState(client, redirectAddress)
	...
	if !bindingReady {
		releasedRedirects := i.cgroupUDPClientTable.setBinding(
			client,
...
func (i *Inbound) newCgroupPacketConnectionEx(ctx context.Context, conn N.PacketConn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	if clientState, loaded := i.cgroupUDPClientTable.load(source.AddrPort()); loaded {
		metadata.UDPConnect = clientState.isConnected()
	}
	i.router.RoutePacketConnectionEx(ctx, conn, metadata, onClose)
}

func (i *Inbound) prepareCgroupPacketConnection(source M.Socksaddr, destination M.Socksaddr, userData any) (bool, context.Context, N.PacketWriter, N.CloseHandlerFunc) {
	connectedUDP, _ := userData.(bool)
	ctx := log.ContextWithNewID(i.ctx)
	client := source.AddrPort()
	clientState := i.cgroupUDPClientTable.loadOrCreate(client)
	clientState.setConnected(connectedUDP, destination.AddrPort())
	writer := &cgroupUDPPacketWriter{
		inbound:     i,
		client:      client,
		clientState: clientState,
	}
	return true, ctx, writer, func(error) {
		i.deleteUDPRedirects(i.cgroupUDPClientTable.delete(writer.client, writer.clientState))
	}
}
...
func (i *Inbound) deleteUDPRedirectsWithBackend(
	...
	for _, redirectAddress := range redirectAddresses {
		redirectDestination := netip.AddrPortFrom(redirectAddress, i.listeners.selectedPort())
		if err := backend.DeleteRedirect(commonEBPF.ProtocolUDP, redirectDestination); err != nil {
...
func (i *Inbound) cgroupSocketControl(ipv6Listener bool) control.Func {
...
type cgroupUDPPacketWriter struct {
	inbound     *Inbound
	client      netip.AddrPort
	clientState *cgroupUDPClientState
}

func (w *cgroupUDPPacketWriter) WritePacket(buffer *buf.Buffer, destination M.Socksaddr) error {
...
func (w *cgroupUDPPacketWriter) reserveReplyBinding(destination netip.AddrPort) (cgroupUDPRedirectBinding, error) {
	if _, available := w.clientState.replyTemplate(destination, false); !available {
		return cgroupUDPRedirectBinding{}, E.New("UDP reply alias limit reached or address family unavailable")
	}
	backend := w.inbound.cgroupBackendInstance()
	if backend == nil {
		return cgroupUDPRedirectBinding{}, E.New("eBPF backend is closed")
	}
	redirectAddress, err := backend.ReserveUDPReplyRedirect(destination, w.inbound.listeners.selectedPort())
	if err != nil {
		return cgroupUDPRedirectBinding{}, err
	}
	released, installed := w.inbound.cgroupUDPClientTable.setReplyBinding(
		w.client,
		w.clientState,
		destination,
		redirectAddress,
	...
	return cgroupUDPRedirectBinding{}, E.New("UDP session closed or reply alias was rejected")
}

func cgroupRedirectAddressFromOOB(oob []byte) (netip.Addr, error) {
	address, _, err := cgroupPacketDestinationsFromOOB(oob)
	return address, err
}

func cgroupPacketDestinationsFromOOB(oob []byte) (netip.Addr, netip.AddrPort, error) {
