...
type sharedNetwork struct {
	inbound              *Inbound
	interfaces           []string
	sharedBackend        *commonEBPF.SharedNetworkBackend
	...
	mapCapacity          commonEBPF.SharedNetworkMapCapacities
...
func effectiveSharedNetworkMapCapacity(
	capacity commonEBPF.SharedNetworkMapCapacities,
	bypassFlowCache bool,
) commonEBPF.SharedNetworkMapCapacities {
	if !bypassFlowCache {
		capacity.Bypass = 1
	}
	return capacity
...
func (s *sharedNetwork) Start(cgroupBackend *commonEBPF.CgroupBackend) error {
...
func (s *sharedNetwork) prepareBackend(cgroupBackend *commonEBPF.CgroupBackend) (*commonEBPF.SharedNetworkBackend, error) {
	backend, err := commonEBPF.PrepareSharedNetwork(cgroupBackend, commonEBPF.SharedNetworkConfig{
...
func (s *sharedNetwork) sharedBackendInstance() *commonEBPF.SharedNetworkBackend {
	s.backendAccess.RLock()
	defer s.backendAccess.RUnlock()
	return s.sharedBackend
}

func (s *sharedNetwork) takeSharedBackend() *commonEBPF.SharedNetworkBackend {
...
func (s *sharedNetwork) setSharedBackend(backend *commonEBPF.SharedNetworkBackend) {
	s.backendAccess.Lock()
...
func (s *sharedNetwork) runFlowJanitor(ctx context.Context, done chan<- struct{}) {
	...
	resetReleaseTimer := func(backend *commonEBPF.SharedNetworkBackend) {
...
func updateSharedFlowPressure(active bool, belowExitRounds int, usage commonEBPF.MapUsage) (bool, int, bool, bool) {
	if usage.Capacity == 0 {
