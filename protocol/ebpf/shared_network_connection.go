...
func (s *sharedNetwork) NewConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	...
	original, flow, err := backend.LookupFlow(commonEBPF.ProtocolTCP, client, tokenDestination)
...
func (s *sharedNetwork) NewPacket(buffer *buf.Buffer, oob []byte, source M.Socksaddr) {
	...
	tokenAddress, _, err := cgroupPacketDestinationsFromOOB(oob)
	if err != nil {
	...
	cached, bindingReady, loaded := s.cgroupUDPClientTable.cachedPacketState(client, tokenAddress)
	original := cached.original
	flow := cached.sharedFlow
	retainedFlow := false
	if !loaded {
		original, flow, err = backend.LookupFlow(commonEBPF.ProtocolUDP, client, tokenDestination)
		if err != nil {
	...
	if !bindingReady {
		released, installed := s.cgroupUDPClientTable.setSharedBinding(client, original, tokenAddress, flow)
...
func (s *sharedNetwork) NewPacketConnectionEx(ctx context.Context, conn N.PacketConn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	if clientState, loaded := s.cgroupUDPClientTable.load(source.AddrPort()); loaded {
...
func (s *sharedNetwork) preparePacketConnection(source M.Socksaddr, destination M.Socksaddr, _ any) (bool, context.Context, N.PacketWriter, N.CloseHandlerFunc) {
	...
	client := source.AddrPort()
	clientState := s.cgroupUDPClientTable.loadOrCreate(client)
	...
	return true, ctx, writer, func(error) {
		s.releaseFlows(s.cgroupUDPClientTable.deleteShared(client, clientState))
	}
}
...
func (s *sharedNetwork) releaseFlow(flow *commonEBPF.SharedNetworkFlowHandle) {
	if flow == nil {
		return
	}
	backend := s.sharedBackendInstance()
...
func (w *sharedPacketWriter) reserveReplyBinding(destination netip.AddrPort) (cgroupUDPRedirectBinding, error) {
	...
	if !loaded {
		return cgroupUDPRedirectBinding{}, E.New("shared-network UDP reply alias limit reached or base flow unavailable")
	}
	backend := w.sharedNetwork.sharedBackendInstance()
	if backend == nil {
		return cgroupUDPRedirectBinding{}, E.New("shared-network eBPF backend is closed")
	}
	...
	released, installed := w.sharedNetwork.cgroupUDPClientTable.setSharedReplyBinding(
		w.client,
		w.clientState,
		commonEBPF.OriginalDestination{Destination: destination, SourceMAC: sourceMAC},
		redirectAddress,
		flow,
	)
	if !installed {
	...
	return cgroupUDPRedirectBinding{}, E.New("shared-network UDP session closed or reply alias was rejected")
}
