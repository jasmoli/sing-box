	...
	go func() {
		...
		for index := 0; index < stressConnections; index++ {
			...
			go func() {
				...
				original, flow, lookupErr := backend.LookupFlow(commonEBPF.ProtocolTCP, client, tokenDestination)
				if lookupErr != nil {
...
func TestSharedNetworkDataPathIntegration(t *testing.T) {
	...
	backend, err := commonEBPF.PrepareSharedNetwork(nil, commonEBPF.SharedNetworkConfig{
		...
		EnableTCP:            true,
		EnableUDP:            true,
		DNSMode:              commonEBPF.DNSModeHijack,
	...
	go func() {
		...
		original, flow, lookupErr := backend.LookupFlow(commonEBPF.ProtocolUDP, client, tokenDestination)
		if lookupErr != nil {
			udpResult <- lookupErr
			return
		}
