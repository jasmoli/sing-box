...
func parseClashSubscription(content string) ([]option.Outbound, error) {
	...
	for _, proxy := range clashConfig.Proxies {
		...
		case "snell":
			outbound, err = newSnellClashParser(proxy)
		case "anytls":
...
func newHysteriaClashParser(proxy map[string]any) (option.Outbound, error) {
	...
	if upRaw, exists := proxy["up"]; exists {
		...
		case string:
			options.Up = stringToNetworkBytes(up)
		case int:
			options.UpMbps = up
		}
	}
	if downRaw, exists := proxy["down"]; exists {
		...
		case string:
			options.Down = stringToNetworkBytes(down)
		case int:
...
func newSnellClashParser(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeSnell,
	}
	...
	if psk, exists := proxy["psk"].(string); exists {
		options.PSK = psk
	}
	if version, exists := proxy["version"].(int); exists {
		options.Version = version
	}
	if obfsOpts, exists := proxy["obfs-opts"].(map[string]any); exists {
		if mode, exists := obfsOpts["mode"].(string); exists {
			options.ObfsOptions.ObfsMode = mode
		}
		if host, exists := obfsOpts["host"].(string); exists {
			options.ObfsOptions.ObfsHost = host
		}
	}
	options.DialerOptions = convertDialerOption(proxy)
	outbound.Options = &options
