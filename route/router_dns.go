	...
	for {
		...
		for _, transport := range transports {
			go func(dnsCtx context.Context, transport dns.Transport) {
				...
				dnsCtx, cancel := context.WithTimeout(dnsCtx, C.DNSTimeout)
				if addressLimit {
					addrs, err = r.dnsClient.LookupWithResponseCheck(dnsCtx, transport, domain, strategy, func(responseAddrs []netip.Addr) bool {
				...
				if len(responseAddrs) > 0 {
					r.dnsLogger.DebugContext(ctx, "lookup succeed for ", domain, ": ", strings.Join(F.MapToString(responseAddrs), " "))
				}
				resChan <- dnsAddr{
					addrs: addrs,
					err:   err,
